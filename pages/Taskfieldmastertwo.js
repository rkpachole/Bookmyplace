...
const Taskfieldmastertwo = () => {
    ...
    const [inactive1, setinactive1] = useState()
    const [notes, setnotes] = useState()
    const [id, setID] = useState()
    const [CustomData1, setCustomData1] = useState([])
    const [datasave1, setData1] = useState([]); // state to hold the API data
    ...
    const inputsearch3 = (e) => {
        if (e.target.value == '') {
            ...
            const filterres = searchapi3.filter(item => item.pyname.toLowerCase().includes(e.target.value.toLowerCase()))
            setCustomData1(filterres)
        }
